/*
	...
	Major Modules : Heapification
			Making Meta data persistent and loading the same at load time of our driver
			Taking the request for the blocks from the user and returning the requested block
...
{
	//Main part 1
	unsigned long long 	n; //Access number
...
struct VD_entry
{
	...
	int segment_number; //Segment number.....Segment is a group of blocks that are considered as a single unit for characterisation and relocation
...
//		printk("<1> \n\n VDno= %d dev_id=%d  seg_no=%d hpindx=%d C.n= %lu C.rdN=%lu C.RFn=%d C.Cn=%d C.Tn=%d C.T*=%lu C.T_prv=%lu C.Ln=%d C.L*=%lu C.RWn=%d C.Rn=%d C.R*=%lu C.T_prv_rd=%lu C.Wn=%d C.W*=%lu C.T_prvWrite=%lu",0, to->device_id, to->segment_number, to->heapindex, (unsigned long)to->C.n, (unsigned long)to->C.readn, (int) to->C.RFn, (int) to->C.Cn, (int) to->C.Tn, to->C.T_star, (unsigned long)to->C.T_previous, (int)to->C.Ln, to->C.L_star, (int)to->C.RWn, (int)to->C.Rn, to->C.R_star, (unsigned int) to->C.T_previous_read, (int) to->C.Wn, to->C.W_star, (unsigned int) to->C.T_previous_write);

	{
		...
		printk("\n After relocation : heap1 index = %d and its dev id = %d heap2index = %d and its dev id = %d new_pos = %d adjust_changed_value_in_heap = %d heap1_pos = %d", from->heapindex, from->device_id, to->heapindex, to->device_id, new_pos, adjust_changed_value_in_heap(heap1_pos_global,Device.heaps[1],Device.heapcounts[1]), heap1_pos_global);
	}
...
//		printk("<1>Segments : %lu", (unsigned long)Device.child_devices[i].size/(SEGMENT_SIZE / KERNEL_SECTOR_SIZE));
		for(j=0;j < no_of_segments;j++)
		{
			...
			printk("[%4d] Dev_ID : %4d Seg_No : %4d\t",k,Device.VDTable[k].device_id,Device.VDTable[k].segment_number);
			k=k+1;
		}
	}
	printk("\n<1>  Total segments= %d \n",k);
